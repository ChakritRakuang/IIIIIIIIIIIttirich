...
public class IconBusinessAdapter extends RecyclerView.Adapter<IconBusinessAdapter.ViewHolder> {
    private Context mContext;

    public IconBusinessAdapter(Context c) {
        mContext = c;
    }

    @Override
    public IconBusinessAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(IconBusinessAdapter.ViewHolder vh, int position) {
        // vh.imgProfile.setImageURI(Uri.parse(mBudgetRows.get(position).getImage()));
        if (position == 0) {
            vh.cateIcon.setImageResource(R.drawable.img_iph_ico_unactive_businese_sale2x);
            vh.cateTag.setText("ร้านค้า");
        }
